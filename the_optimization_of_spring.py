...
def the_optimization_of_spring (n, legs, g, M, V_0, k, G, p, l):
    ...
    #Расчёт эквивалентной массы.
    M /= legs
    #Расчёт максимальной высоты, выше которой невозможно будет подобрать пружинну.
    y_max = l + (M * g * (n**2 - 1)) / (2 * k) - V_0**2 / (2 * g)
    print(f"Your hight must be less, then {y_max} meters.")
    ...
    if y < y_max:
        ...
        #Расчёт максимального диаметра проволоки пружины, больше которого пружина не сможет обеспечить нужную перегрузку.
        d_max = 2 * ((2 * beta**2 * k * M * (M * g**2 * (n**2 - 1) + k * (2 * g * (l - y) - V_0**2))) / (pi**2 * p * G * (M * g**2 * (n + 1) + k * (2 * g * y + V_0**2))))**0.25
        print(f"Your diameter should be less, then {d_max} meters.")
        d = float(input("Choose diameter, that you want"))
        if d <= d_max:
            #Расчёт массы пружины.
            print(f"Your spring will have {round((pi**2 * G * p * d**4) / (32 * beta**2 * k), 4)} kilogram.")
            #Расчёт диаметра пружины.
            print(f"Your spring will have {beta * d} meters of diameter of it.")
            #Расчёт количества витков пружины.
            print(f"Your spring will have {round((G * d) / (8 * beta**3 * k), 4)} rounds.")
        else:
